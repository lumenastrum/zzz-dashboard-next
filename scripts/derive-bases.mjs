...
import { discAccum, setSheetAccum, weaponAdvanceAccum, resolveWengine, computeSheet } from "../src/lib/grading/grading.js";
...
for (const agent of data.agents) {
  ...
  const wa = weaponAdvanceAccum(resolveWengine(agent.wengine, cfg));
  // invert per stat, removing disc + sheet-scope set + MULTIPLICATIVE W-Engine-advanced contributions, so
  // base = pure character (+ core) only for multiplicative stats (full precision — computeSheet reproduces
  // the seed exactly). Extracting the engine advanced % is what makes ATK/HP/AM/Impact/ER correct when disc
  // %s are edited (else the advanced % stays multiplied into base and the sum-vs-product mismatch drifts).
  agent.base = {
    "ATK": (s["ATK"] - d.flatAtk - ss.flatAtk) / (1 + (d.atkPct + ss.atkPct + wa.atkPct) / 100),
    "HP": (s["HP"] - d.flatHp - ss.flatHp) / (1 + (d.hpPct + ss.hpPct + wa.hpPct) / 100),
    "DEF": (s["DEF"] - d.flatDef - ss.flatDef) / (1 + (d.defPct + ss.defPct + wa.defPct) / 100),
    ...
    "Anomaly Mastery": (s["Anomaly Mastery"] ?? 0) / (1 + (d.amPct + ss.amPct + wa.amPct) / 100),
    "Impact": (s["Impact"] ?? 0) / (1 + (d.impactPct + ss.impactPct + wa.impactPct) / 100),
    "Energy Regen": (s["Energy Regen"] ?? 0) / (1 + (d.erPct + ss.erPct + wa.erPct) / 100),
